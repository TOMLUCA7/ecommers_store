  ...
  return (
    <div className="container lg:max-w-screen-lg mx-auto py-12 px-6">
      <div className="flex flex-col md:flex-row justify-between items-center space-y-8 md:space-y-0 md:space-x-12">
        <div className="flex relative w-72 h-72 sm:w-96 sm:h-96">
          <Image
            src={product.image}
            alt={product.name}
            fill
        ...
        <div className="w-full flex-1 max-w-md border-opacity-50 rounded-md shadow-lg p-6 bg-white">
...
export async function getStaticProps({ params }) {
  ...
  const FindProduct = products.data.find((product) => product.id === params.id);

  const price = FindProduct.default_price;
  const productData = {
